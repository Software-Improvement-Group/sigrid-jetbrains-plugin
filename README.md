...
- **Fix with AI** — hand selected findings to [Claude Code](https://claude.com/product/claude-code) via the **Fix with AI** toolbar button or the **Fix it with Claude Code** context menu item; a prompt built from the findings (and Sigrid MCP slash commands, when the Sigrid MCP plugin is installed) is sent to a `claude` terminal session.
