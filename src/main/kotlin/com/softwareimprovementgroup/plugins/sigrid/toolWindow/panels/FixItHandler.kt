...
class FixItHandler<T>(
    ...
    /** Nudges the user towards the agent's MCP install flow once per session, never blocking. */
    private fun notifyMcpNotDetected(agent: AiAgentProvider) {
        val hint = agent.getMcpInstallHint() ?: return
        if (!shownMcpHints.add("${agent.id}:mcpInstall")) return

        val notification = NotificationGroupManager.getInstance()
            .getNotificationGroup(NOTIFICATION_GROUP_ID)
            .createNotification(hint.message, NotificationType.WARNING)
        notification.addAction(NotificationAction.createSimple(hint.action) {
